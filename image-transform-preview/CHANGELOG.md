...
### ⚡ パフォーマンス最適化: mergeImages関数の高速化

#### 概要
画像合成処理（mergeImages）のパフォーマンスを大幅に改善しました。

#### ループ最適化
- `getPixelPtr`呼び出しを内側ループから外側ループに移動
  - 変更前: O(W×H) 回の呼び出し（毎ピクセル）
  - 変更後: O(H) 回の呼び出し（毎行）
- y/x範囲を事前計算し、内側ループの条件分岐を削減
- 単純なポインタ演算で高速化

#### アルファ値による最適化
- `srcA == 0`: ピクセルスキップ（完全透明のソースは処理不要）
- `srcA == 65535 || dstA == 0`: 単純コピー（乗算不要）
- 条件付き加算パターンで分岐を統合し、代入処理を一箇所に統一

#### コード構造
```cpp
if (srcA == 0) continue;  // 透明スキップ
...
#### コミット
- `8958a6a` - perf: Optimize mergeImages with loop and alpha optimizations
