...
### Image・Composite ノードのパフォーマンス計測を追加

**内容**
- ImageEvalNode に処理時間計測を追加（画像データコピー含む）
- CompositeEvalNode に処理時間計測を追加（上流評価時間を除外、合成処理のみ）

これで全5種類のノードタイプ（Image, Filter, Affine, Composite, Output）がデバッグビルドでパフォーマンス計測を報告。

---

### サイドバーにデバッグ詳細セクションを追加

**内容**
- サイドバーのアコーディオンに「🐛 デバッグ」セクションを追加
- ノードタイプ別の処理時間・呼び出し回数・メモリ確保量を表示
- ステータスバークリックでデバッグセクションを開く機能
- ドキュメントにデバッグビルド推奨の記述を追加

**注意**: パフォーマンス計測にはデバッグビルド（`./build.sh --debug`）が必要

---

### サイドバーUIをアコーディオン形式に変更

**背景**
- スマートフォンでサイドバーの「出力設定」セクション下部が画面外にはみ出る問題
- `100vh`がモバイルブラウザのUI（アドレスバー等）を考慮しない問題

**設計変更**
- サイドバーを「画像ライブラリ」「出力設定」のアコーディオン形式に変更
- 同時に1つのセクションのみ展開可能（排他的アコーディオン）
- デフォルトは「画像ライブラリ」が展開
- 高さに `100dvh` と `-webkit-fill-available` を追加（モバイル対応）

**利点**
- スマートフォンで全設定項目にアクセス可能
- 将来のセクション追加にも対応可能な設計
- ヘッダーの色切り替えがスムーズにアニメーション
