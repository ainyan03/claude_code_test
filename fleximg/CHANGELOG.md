...
- **SourceNode AABB拡張**: バイリニア補間時のフェードアウト領域をAABBに反映
  - edgeFadeFlags有効な辺ごとに入力矩形を0.5ピクセル拡張してからAABBを計算
  - `calcAffineAABB` の引数をint→floatに変更（小数精度の拡張に対応）
...
- **m5stack_basic デモ**: バイリニア補間のグローバル切替機能を追加
  - ボタンAダブルクリックで全モードのバイリニア有効/無効を切替可能に
  - UI上部に `Bilinear: ON/OFF` 表示を追加
  - `1 Bilinear` モードを `1 Zoom` に改名（拡大率10倍でバイリニア効果確認用）
...
- **EdgeFadeFlags**: バイリニア補間の辺ごとのフェードアウト制御
  - `EdgeFade_Left/Right/Top/Bottom` で各辺のフェードアウトを個別制御
  - `SourceNode::setEdgeFade()` APIを追加
  - NinePatch内部エッジでのフェードアウト無効化に使用

### 改善

- **NinePatchSourceNode**: パッチ合成処理の改善
  - transferFrom（ブレンド）からplaceFirst（上書き）に変更
  - 描画順序を「伸縮→固定」に変更（バイリニア時のエッジ軽減）
  - 固定パッチが全体サイズを超えないよう比率縮小
  - 伸縮パッチのマイナスサイズを許容（反転表示で予測可能な挙動）
...
- **境界ピクセル処理の改善**: アルファチャネルのみゼロ埋めに変更
  - 旧: 境界ピクセル全体(RGBA)をゼロクリア → 黒いフリンジが発生
  - 新: アルファチャネルのみゼロ埋め → RGB値を据え置き、自然なエッジ処理
  - バイリニア補間時の境界領域で自然なカラーフェードアウトを実現
...
- **プール系クラス**: acquire/releaseの簡素化とスレッド安全性向上
  - RenderContext: `setup()`メソッド追加、個別setterを廃止
  - 解放順序を「リソース解放→inUse=false」に統一（スレッド安全）
  - ループカウンタを`uint_fast8_t`に統一

- **ImageBufferEntryPool**: 二重解放防止チェックを追加
  - `release()`でinUseチェック後にリソース解放

- **ImageBufferSet**: 初期化処理の重複排除
  - メンバ変数にデフォルト初期化子を追加
  - 3つのコンストラクタからループを削除
  - `insertOrMerge()`を簡素化（エントリ数半分超で積極的統合）
...
- **ImageBufferSet**: エントリ上限超過時の自動統合機能を追加
  - `insertSorted()`を単一責務（挿入のみ）に簡素化
  - 上限超過時は`mergeAdjacent()`→`consolidateInPlace()`で自動的に空きを作成
  - 統合後のオフセット復元により正しい座標を維持

### リファクタリング

- **ImageBufferSet**: `insertOrMerge()`ヘルパーを追加
  - `addBuffer()`と`transferFrom()`の重複コードを共通化
  - 重複チェック→挿入/マージ→失敗時統合再試行を一元管理
  - コード量を18行削減

### 設定変更

- **ImageBufferSet::MAX_ENTRIES**: 32→8に削減
  - メモリ制約環境向けの調整
  - 統合機能により少ないエントリ数でも正常動作
