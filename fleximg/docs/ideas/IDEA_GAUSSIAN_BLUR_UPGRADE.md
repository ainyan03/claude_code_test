...
32bit演算でのオーバーフロー防止のため、以下の上限を設けています：
...
- passes=3, radius=19 の場合: maxSumRGB ≈ 3.86×10^9 < 4.29×10^9 ✓

---

## メモリ消費量（VerticalBlurNode）

垂直ブラーはパイプライン方式のスキャンラインキャッシュを使用します（2026-01-19更新）。
...
**組み込み環境（ESP32等）での目安**:
- QVGA (320幅): radius=19, passes=3 で約165KB
- VGA (640幅): radius=19, passes=3 で約330KB
