...
**問題**: 45度回転等の場合、出力タイル（例: 256×1）を逆変換したAABBが巨大化（約181×181）し、無駄な領域を上流に要求してしまう。

**アイデア**: 入力AABBをY方向に分割し、各Y範囲で本当に必要なX範囲のみを要求する。
...
- [ ] パフォーマンス測定基盤の整備
- [ ] 分割判定ロジック（分割の価値がある場合のみ実行）
- [ ] 固定分割（例: 4分割）での実装
- [ ] 効果測定と最適な分割戦略の検討

**期待効果**: 45度回転時に約67%のピクセル要求削減

**懸念事項**:
- 回転角度が軸に近い場合は分割不要（オーバーヘッドのみ）
